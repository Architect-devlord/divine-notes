...
[[wiki/codebase/files/chat_system|chat_system.py]] — **a genuine open architectural question, not resolved this pass**: this file routes GUI/game WebSockets to `agent.brain` defensively, via `hasattr()` checks and fallbacks, as if `agent.brain` might not always implement `process_language_input`/`learn_from_file`/`should_speak`/`generate_speech` — but everything else read in this ingest confirms it always does. Also runs its own independent autonomous-speech loop, separate from [[cognitive_loop]]'s. Whether `main.py` (not yet read) actually wires this in as the live routing path, or whether it's superseded by `agent.py`'s own direct routes and `communication_protocol.py`, is worth a direct question rather than a guess.
