...
In other words: drop an agent's BrainCapsule folder into `dw_agents_sim/`, and within 5 seconds it's a live agent inside Isaac Sim, no manual wiring required. This is a working, code-level instance of the design wiki's "Agent Import/Export System" concept (`Robots.md`) — for the Isaac Sim direction specifically. The reverse direction (does a sim session write an updated `brain.pcap` back out automatically, or is that a manual step?) wasn't confirmed in this pass — worth a follow-up read if the full round-trip matters for a specific question.
