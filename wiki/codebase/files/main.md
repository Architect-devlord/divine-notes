...
- `start_agent_process(agent_id, mode, server_addr, load_brain, additional_args, agent_type, custom_name, memory_mb, gender)` — the core spawn path. If a packaged executable already exists at `npc_applications/<id>/<id>` (no `DW_` prefix — matches `packager.py`'s own `exe_name` convention), runs that directly — **and, as of the 2026-07-16 pull, now also reads `backend_port` back from that packaged agent's own `config.json`, rather than leaving it unset.** Previously, this branch never computed `agent_info[agent_id]["backend_port"]` at all, so it silently stayed `0` for any agent launched from a pre-packaged exe — quietly breaking `/api/agents/chat_heard`'s new forwarding lookup (above) for exactly that case, since that lookup depends on this exact field. Otherwise resolves a username/UUID, registers the agent via `server_integration.register_agent(...)` (now passing `gender` as a real parameter — see Problems/Solutions, a dead-parameter bug fixed here), resolves stable TCP/WS ports from `agents.json` (registering a new name if one isn't found yet), and launches `ai_core/agent.py` as a subprocess with `--agent-id`/`--mode`/`--port`/`--tcp-port`/`--log-level`/`--brain-save-path` and optionally `--custom-name`/`--load-brain`/whatever `additional_args` the caller supplied. Schedules two background tasks: `_monitor_logs()` and `_auto_package_agent()`. Deliberately does **not** call UltimMC setup/launch itself — see the next method for why.
...
- `POST /api/agents/chat_heard` — **fixed upstream, 2026-07-16 pull.** Previously called `agent_manager.get_chat_queue()`, a method that never existed anywhere — confirmed broken on this page's first write-up, now confirmed fixed by direct diff against the new commit, not assumed from a changelog. The route no longer tries to hold agent state in this process at all (which was the deeper problem — every agent is its own subprocess, so an in-memory queue here was never reachable from anywhere). It now looks up `hearer_id`'s `backend_port` from `agent_manager.agent_info` and forwards the event over HTTP to that agent's own new `POST /api/perception/chat_heard` route (see [[agent]]). **The docstring was rewritten too, and it now says almost exactly what this page independently concluded a few entries ago** (see "The chat_system.py question, resolved," and the correction recorded on [[chat_system]] and [[oracle-two-systems]]): `LLMOracleBrain` is real, but it's a Java-side, Ollama-backed dialogue layer for Oracle teaching/Q&A, not a replacement brain — every god, Oracle included, runs the same `agent.py`/`BrainCore` pipeline as every NPC. Also newly confirmed by the updated docstring: this endpoint is the _only_ delivery channel god agents have at all (no client-side chat listener), and merely a faster supplementary channel for NPCs, who also get the same event slightly later via `/ws/agent` — and `OracleSystem.java` reuses this exact call for `/oracle teach` delivery, confirmed independently this session by reading that file directly before this fix was even pulled.
    
    A second, related fix landed in the same pull: `start_agent_process()`'s packaged-executable branch never computed a `backend_port` for `agent_info`, so it silently stayed `0` for any agent launched from a pre-packaged exe — breaking exactly the lookup this route depends on for that case. Now reads it back from the packaged agent's own `config.json` (written there by [[wiki/codebase/files/packager|packager.py]]'s `_create_config()`), with a graceful `{"status": "ignored"}` if it's still unavailable rather than a crash.
...
1. **Packaging.** `py_backend/config.py`'s `AGENT_EXCLUDE_MODULES` explicitly excludes `py_backend.chat_system` from the packaged agent build (comment: _"server-side router, not agent"_) — confirmed identical in `ai_core/config.py`'s copy too. This entry was consistent between the two copies even before the 2026-07-16 pull fixed [[config-two-copies]]'s separate `agent_spawner` mismatch finding — that inconsistency has since been fixed too, so both entries now agree between the copies, not just this one. `cognitive_loop.py` itself **is** bundled into every packaged agent. So in a packaged, standalone-exe agent, this import fails and the `except Exception: pass` silently swallows it.
...
**A nuance that raised a real question, now settled — corrected 2026-07-05, and independently confirmed by an upstream fix on 2026-07-16**: `/api/agents/chat_heard`'s docstring (this file) states god agents run a separate `LLMOracleBrain`, distinct from the `BrainCore`/`LanguageIntelligence` combination confirmed everywhere else in this ingest. This page originally floated that as a plausible explanation for [[chat_system]]'s `hasattr()` defensiveness, since every NPC-side brain this ingest touched always implements `process_language_input`/`should_speak`/etc. It doesn't hold up. `LLMOracleBrain` (`DivineWorld/oracle/LLMOracleBrain.java`, now read directly) is System B per [[oracle-two-systems]] — an Ollama-based tutor spawned per human player, unrelated to `NPCAgent`/`BrainCore` entirely. Every god agent, including the Oracle god type, uses the same `BrainCore` as every NPC, per that page's own System A section, already written before this docstring was read. The `hasattr()` defensiveness remains genuinely unexplained by anything found this ingest — back to an open question, not a solved one. Worth being direct about the miss: [[oracle-two-systems]] was cited by name during this file's original ingest (for the unrelated `oracle_stuff/` deprecation fact) without being read closely enough to catch that it already covered `LLMOracleBrain` specifically. **Satisfying confirmation, not just a coincidence of timing**: the actual docstring in the repo was rewritten in the same pull that fixed `get_chat_queue()`, and it now says almost exactly this — `LLMOracleBrain` is a dialogue layer for Oracle teaching, not a brain replacement, every god runs ordinary `BrainCore`. Whether this wiki's write-up informed that fix or the two were arrived at independently isn't something this page can determine, but the conclusions match closely enough to be worth noting either way.
