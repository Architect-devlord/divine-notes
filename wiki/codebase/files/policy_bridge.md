...
- `__init__(transformer_policy, cl_policy_net, obs_dim, action_dim)` — probes for a usable encoder once (see `_find_encoder()` below) and builds `cl_head` (a 2-layer MLP) sized to whatever that probe returns.
