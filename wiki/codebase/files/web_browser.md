...
- `agent.py`'s own `/browser/*` routes (`navigate`, `click`, `type`, `scroll`, `screenshot`, `stats`, `history`, `allowed_sites`) — per that file's own comment, deliberately live on the per-agent app rather than `main.py` "because the browser instance lives inside `NPCAgent`." **Both previously-broken routes are fixed as of the 2026-07-16 pull** (confirmed by diff against the upstream repo, not assumed from a changelog): `GET /browser/screenshot` was calling `web_browser.screenshot_jpeg()`, a method that never existed anywhere on this class — now calls the real `screenshot()` (PNG). `POST /browser/scroll` was gating its logic behind `hasattr(web_browser, "_page")`, always `False` since `WebBrowser` never sets that attribute (every interaction method uses a local `page` variable, closed before returning) — now calls `scroll()` directly, the real method that had been sitting unused. The fix comment in the source names the same root cause this page already had, independently.
