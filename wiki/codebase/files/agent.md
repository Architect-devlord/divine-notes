...
- `POST/GET /browser/navigate|click|type|scroll|screenshot|stats|history|allowed_sites` — direct pass-through to **`agent.web_browser`** (corrected 2026-07-05 — this page previously said `agent.browser`; every real call site, in this same file, uses `web_browser`), a `WebBrowser` instance — see [[web_browser]]. **Both previously-broken routes are fixed as of the 2026-07-16 pull, confirmed by diff, not assumed**: `GET /browser/screenshot` now calls `web_browser.screenshot()` (the real method — it was calling a nonexistent `screenshot_jpeg()` before, a straight name fix), and `POST /browser/scroll` now calls `web_browser.scroll(url, direction, amount)` directly instead of gating dead logic behind a `hasattr(..., "_page")` check that was always `False` — the fix comment in the source names the exact same root cause this page already had: `WebBrowser` never sets `self._page`, since every interaction method opens and closes its own local page. Full detail, including the original bug write-up for context, on [[web_browser]].
- `POST /api/perception/chat_heard` — **new as of the 2026-07-16 pull**, the receiving end of `main.py`'s now-fixed `/api/agents/chat_heard` forwarding. Takes `speaker`/`message`/`timestamp`, delivers into `agent.brain`'s language pipeline the same way any other overheard speech would. See [[main]] for the full chat_heard fix, and [[oracle-two-systems]] for `/oracle teach`'s dependency on this same chain.
