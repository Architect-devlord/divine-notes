...
- **`familiarity_r`** — a repeat-visitor bonus keyed by `speaker_id`, added specifically so a stable per-installation visitor id (sent from the web/Electron frontend) lets an agent build genuine familiarity with someone it's talked to before, rather than treating every chat as a stranger. **Fixed, 2026-07-05**: the underlying `_partner_visit_counts`/`_partner_last_seen` tracking in [[wiki/codebase/files/brain_language|brain_language.py]] was pure in-RAM state with no save path at all — every agent restart silently reset every returning visitor back to "first visit," even though the full conversation history itself survived fine in ScyllaDB. Now included in `LanguageIntelligence.state_dict()`.
