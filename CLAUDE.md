...
**Naming collisions between two real files sharing a filename**: if the codebase itself has two files with the identical name at different paths (e.g. `py_backend/config.py` and `py_backend/ai_core/config.py`), don't force the one-page-per-file rule — a single combined page, named to signal its dual nature (`config-two-copies.md`, not just `config.md`), documenting both and their actual divergence, serves the reader better than two 90%-identical pages. This is itself sometimes the finding worth documenting, not just a naming inconvenience to work around quietly.
...
- `AUDIT_REPORT.md`'s critical duplicate-Config-class finding — **confirmed and explained, not fixed**, 2026-07-05. See [[wiki/codebase/files/config-two-copies|config-two-copies]]: it's not a stale copy, it's two files serving two genuinely different runtime contexts (`py_backend.*`-prefixed for anything invoked from the repo root, bare `ai_core.*`/`rl.*` for a packaged agent executable), plus one real, verified inconsistency in the `agent_spawner` exclusion list that doesn't match either copy's own comment. Whether that inconsistency is currently load-bearing wasn't traced — this is documentation of the finding, not a code change to the actual files.
